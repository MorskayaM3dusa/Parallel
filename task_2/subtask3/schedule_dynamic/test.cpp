...
void solving(double *A, double *x_previous, double *b, long long n, double tau, double epsilon, int num_of_threads)
{
    ...
    double error = 1e10;
    double b_norm = 0;
#pragma omp parallel for schedule(dynamic, n / num_of_threads) num_threads(num_of_threads)
...
#pragma omp parallel num_threads(num_of_threads)
        {
            double middle_norm = 0;
#pragma omp for schedule(dynamic, n / num_of_threads)
            for (long long i = 0; i < n; i++)
            {
                middle_res[i] = 0.0;
...
#pragma omp atomic
                middle_norm += middle_res[i] * middle_res[i];
